...
const WEB_SERVER_PORT = process.env.SERVER_PORT || 18000;
    ...
    if (
      ...
      ((data.mode === 2 || data.mode === 3) &&
        data.response_list.find(
          (response) => !response.valid_signature || !response.valid_ial
        ) == null)
    ) {
      if (data.status === 'completed') {
        if (data.data_request_list && data.data_request_list.length > 0) {
